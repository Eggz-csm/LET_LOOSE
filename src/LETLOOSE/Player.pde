 ...
 void update(ArrayList<Platform> platforms) {
  ...
  void display() {
    ...
    if (gettingUp) {
      ...
      if (faceRight) {
        if (f == 1) image(getup1, x + 10, y, spriteW, spriteH);
        else if (f == 2) image(getup2, x + 10, y, spriteW, spriteH);
        else if (f == 3) image(getup3, x + 10, y, spriteW, spriteH);
        else { f = 1; gettingUp = false; }
      } else {
        scale(-1, 1);
        if (f == 1) image(getup1, -x + 5, y, spriteW, spriteH);
        else if (f == 2) image(getup2, -x + 5, y, spriteW, spriteH);
        else if (f == 3) image(getup3, -x + 5, y, spriteW, spriteH);
        else { f = 1; gettingUp = false; }
      }
    } 
    // --- Running / idle animation ---
    else {
      if (moveLeft) { scale(-1, 1); image(runGif, -x, y, spriteW, spriteH); faceRight = false; }
      else if (moveRight) { image(runGif, x, y, spriteW, spriteH); faceRight = true; }
      else {
        if (faceRight) image(still, x, y, spriteW, spriteH);
        else { scale(-1, 1); image(still, -x, y, spriteW, spriteH); }
      }
    }
    popMatrix();
